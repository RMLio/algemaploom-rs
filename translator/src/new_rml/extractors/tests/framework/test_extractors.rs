use crate::new_rml::rml_model::v2::core::{
    ...
    expression_map::term_map::SubjectMap
};
...
fn get_subject_map(tm: &TriplesMap) -> Result<&SubjectMap, String> {
    tm.subject_map.try_unwrap_subject_map_ref()
...
fn get_pom(tm: &TriplesMap, index: usize) -> Result<&PredicateObjectMap, String> {
    tm.predicate_object_map_vec.get(index)
        .ok_or(format!("No predicate-object map at index {}", index))
}

fn get_expression_value_from_expression_map(expression_map: &crate::new_rml::rml_model::v2::core::expression_map::ExpressionMap) -> Result<String, String> {
    expression_map.get_value()
        .ok_or("No value expression".to_string())
        .map(|s| s.clone())
}

fn get_expression_value(term_map: &TermMapEnum) -> Result<String, String> {
    get_expression_value_from_expression_map(&term_map.as_ref().expression)
}
...
fn stringify_rcterm<T: sophia_api::prelude::Term + Clone>(term: &T) -> Result<String, String> {
...
pub fn extract_triples_map_identifier(triplesmap: &TriplesMap) -> Result<String, String> {
    stringify_rcterm(&triplesmap.identifier)
}

pub fn extract_predicate_object_maps_count(triplesmap: &TriplesMap) -> Result<usize, String> {
    Ok(triplesmap.predicate_object_map_vec.len())
}

pub fn extract_logical_source_iterator(triplesmap: &TriplesMap) -> Result<String, String> {
    let ls = get_logical_source(triplesmap)?;
    ls.iterable.iterator.clone().ok_or("No iterator defined".to_string())
}

pub fn extract_logical_source_reference_formulation(triplesmap: &TriplesMap) -> Result<String, String> {
    let ls = get_logical_source(triplesmap)?;
    ls.iterable.reference_formulation.as_ref()
...
pub fn extract_subject_map_template(triplesmap: &TriplesMap) -> Result<String, String> {
    let sm = get_subject_map(triplesmap)?;
    get_expression_value_from_expression_map(&sm.term_map_info.expression)
}

pub fn extract_subject_map_has_classes(triplesmap: &TriplesMap) -> Result<bool, String> {
    let sm = get_subject_map(triplesmap)?;
    Ok(!sm.classes.is_empty())
}

pub fn extract_subject_map_first_class(triplesmap: &TriplesMap) -> Result<String, String> {
    let sm = get_subject_map(triplesmap)?;
    let class = sm.classes.first().ok_or("No classes defined")?;
    stringify_rcterm(class)
}

pub fn extract_subject_map_term_type(triplesmap: &TriplesMap) -> Result<String, String> {
    let sm = get_subject_map(triplesmap)?;
...
pub fn extract_source_path(triplesmap: &TriplesMap) -> Result<String, String> {
    ...
    match get_object(&source.kind.metadata, &source.kind.subj_iri, &path_predicate) {
        Ok(path_term) => {
...
pub fn extract_source_root(triplesmap: &TriplesMap) -> Result<String, String> {
    ...
    match get_object(&source.kind.metadata, &source.kind.subj_iri, &root_predicate) {
        Ok(root_term) => {
...
pub fn extract_has_ref_object_map_in_pom(triplesmap: &TriplesMap, pom_index: usize) -> Result<bool, String> {
    let pom = get_pom(triplesmap, pom_index)?;
    Ok(!pom.ref_object_map.is_empty())
}

pub fn extract_ref_object_map_parent_triples_map(triplesmap: &TriplesMap, pom_index: usize) -> Result<String, String> {
    let pom = get_pom(triplesmap, pom_index)?;
...
pub fn extract_join_condition_child(triplesmap: &TriplesMap, pom_index: usize) -> Result<String, String> {
    let pom = get_pom(triplesmap, pom_index)?;
    if pom.ref_object_map.is_empty() {
        return Err("No RefObjectMap found in this POM".to_string());
    }
    ...
    join_condition.child.get_value()
        .ok_or("No child value in join condition".to_string())
        .map(|s| s.clone())
}

pub fn extract_join_condition_parent(triplesmap: &TriplesMap, pom_index: usize) -> Result<String, String> {
    let pom = get_pom(triplesmap, pom_index)?;
    if pom.ref_object_map.is_empty() {
        return Err("No RefObjectMap found in this POM".to_string());
    }
    ...
    let join_condition = &ref_obj_map.join_condition[0];
    join_condition.parent.get_value()
        .ok_or("No parent value in join condition".to_string())
        .map(|s| s.clone())
}
