...
impl TermMapExtractor<TermMapEnum> for GraphMap {
    fn create_shortcut_map(
        ...
        if term_map_info.is_literal_term_type() {
            return Err(ParseError::GenericError("Constant-valued GraphMap has to be either an IRI or a BlankNode".to_string()));
        }
        Ok(TermMapEnum::GraphMap(Self { term_map_info }))
    }
        ...
        TTerm: Term + Clone,
    {
        ...
        if term_map_info.is_literal_term_type() {
            Err(ParseError::GenericError("GraphMap has to have a term type of either an IRI, UnsafeIRI, URI, UnsafeURI or a BlankNode".to_string()))
        } else {
